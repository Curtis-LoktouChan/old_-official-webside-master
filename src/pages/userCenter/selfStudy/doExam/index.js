...
function getExamReq(params) {
  ...
  return new Promise((resolve, reject) => {
    http("get", "/api/v1/selfStudy/getExam", params, token).then(
      (res) => {
        resolve(res);
      },
      (error) => {
...
function DoExam(props) {
  ...
  const confirmSubmit = () => {
    submitExamReq(state.data).then((res) => {
      ...
      if (props.roleId === "2") {
        history.push("/userCenter/selfStudy/scoreResult", {
          knowledge_list: [],
        });
  ...
  useEffect(() => {
    getExamReq({ subject: selfStudySubject })
      .then((res) => {
  ...
  return (
    <div>
      ...
      <Form scrollToFirstError onFinish={confirmSubmit}>
        <Row>
          <Col offset={2}>{ExamQuestions(examState, dispatch, classID)}</Col>
        </Row>
        <Row align="center">

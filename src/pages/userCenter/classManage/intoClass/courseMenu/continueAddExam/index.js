...
function continueAddExamReq(params) {
  const token = localStorage.getItem("token");
  return new Promise((resolve, reject) => {
    http("post", "/api/v1/teacher/continueAddExam", params, token).then(
      (res) => {
        resolve(res);
      },
      (error) => {
...
function ContinueAddExam(props) {
  const history = useHistory();
  ...
  return (
    <div style={{ margin: "0px 20px" }}>
      ...
      <Row style={{ border: "1px solid #96cefb5c" }} justify="center">
        <Col span={3}></Col>
        <Col span={18}>
          <ExamCard sectionID={sectionID} classID={classID} />
        </Col>
        <Col span={3}></Col>
      </Row>
    </div>
...
export default connect(mapStateToProps)(ContinueAddExam);

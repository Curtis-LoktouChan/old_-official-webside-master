...
function getExamReq(params) {
  ...
  return new Promise((resolve, reject) => {
    http("get", "/api/v1/teacher/getExam", params, token).then(
      (res) => {
        resolve(res);
      },
      (error) => {
...
function submitExamReq(params) {
  const token = localStorage.getItem("token");
  return new Promise((resolve, reject) => {
    http("post", "/api/v1/teacher/submitExam", params, token).then(
      (res) => {
        resolve(res);
      },
      (error) => {
...
function ViewExam(props) {
  ...
  const confirmSubmit = () => {
    submitExamReq(state.data).then((res) => {
      history.push("/userCenter/intoClass/scoreResult", { sectionID });
    });
  };
